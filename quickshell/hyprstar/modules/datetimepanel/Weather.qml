...
Rectangle {
    ...
    property var dailyHiC: []
    property var dailyLoC: []
    property int dailyCount: 0
    readonly property int shownDays: Math.min(dailyCount, 10)
    readonly property real todayHiC: (dailyCount > 0 && dailyHiC.length > 0) ? Number(dailyHiC[0]) : NaN
    readonly property real todayLoC: (dailyCount > 0 && dailyLoC.length > 0) ? Number(dailyLoC[0]) : NaN
    ...
    function fetchWeather() {
        ...
        if (!isNaN(tempC)) condition = "Updating…"
        meteoProc.buffer = ""
        ...
        Item {
            ...
            Column {
                ...
                Row {
                    ...
                    Text {
                        text: (isNaN(root.todayHiC) || isNaN(root.todayLoC))
                              ? "High --°  •  Low --°"
                              : ("High " + Math.round(root.todayHiC) + "°  •  Low " + Math.round(root.todayLoC) + "°")
                        ...
                        delegate: Item {
                            ...
                            RowLayout {
                                ...
                                Text {
                                    ...
                                    verticalAlignment: Text.AlignVCenter
                                    text: (root.dailyHiC.length > index ? Math.round(Number(root.dailyHiC[index])) : "--") + "°"
                                    color: Theme.Theme.text
                                    font.pixelSize: 13
                                    font.weight: 700
                                    font.family: "Adwaita Sans"
                                ...
                                Text {
                                    ...
                                    horizontalAlignment: Text.AlignRight
                                    verticalAlignment: Text.AlignVCenter
                                    text: (root.dailyLoC.length > index ? Math.round(Number(root.dailyLoC[index])) : "--") + "°"
                                    color: Theme.Theme.subText
                                    font.pixelSize: 13
                                    font.weight: 700
                                    font.family: "Adwaita Sans"
    ...
    Process {
        ...
        onRunningChanged: {
            ...
            try {
                ...
                root.tempC = Number(cur.temperature_2m)
                ...
                root.dailyHiC   = daily.temperature_2m_max || []
                root.dailyLoC   = daily.temperature_2m_min || []

                root.dailyCount = Math.min(
                    root.dailyTimes.length, root.dailyCode.length,
                    root.dailyHiC.length, root.dailyLoC.length, 10
                )
